"""
utils/market_data.py — cotações yfinance + câmbio
"""
...
def get_taxas_cambio(moeda_base: str = "BRL") -> dict:
    """
    Retorna rates onde rates[X] = quanto de X vale 1 moeda_base.
    Ex: base=BRL → rates["USD"]=0.177 → 1 BRL = 0.177 USD → 1 USD = 5.65 BRL
    """
    try:
        r = requests.get(f"https://api.exchangerate-api.com/v4/latest/{moeda_base}", timeout=6)
    ...
    if moeda_base == "BRL":
        return fb
    br = fb.get(moeda_base, 1.0)
    return {k: v/br for k,v in fb.items()}


def taxa_brl_por_moeda(moeda: str, taxas_brl: dict) -> float:
    """Quantos BRL valem 1 unidade de `moeda`."""
    if moeda == "BRL":
        return 1.0
...
def converter_valor(valor: float, de: str, para: str, taxas_brl: dict) -> float:
    ...
    if para == "BRL":
        return em_brl
    return em_brl * taxas_brl.get(para, 1.0)
...
def get_cotacao(ticker: str) -> dict:
    ...
    try:
        ...
        var   = ((preco - prev) / prev * 100) if preco and prev and prev != 0 else 0.0
        moeda = (getattr(info, "currency", "BRL") or "BRL").upper()
        return {"preco": round(float(preco), 4) if preco else None,
                "variacao_dia": round(float(var), 2), "moeda": moeda}
    except Exception:
        return {"preco": None, "variacao_dia": 0.0, "moeda": "BRL"}
